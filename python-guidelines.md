...
- **House uv-init defaults — this stanza is the owning source; keeping these scaffold outputs is derivation, not a judgment call to declare:** a new package keeps `uv init`'s generated `version = "0.1.0"` (until a release process rules otherwise), the `uv_build` build backend, and the `py.typed` marker. A scaffold output not named in this stanza remains an unowned fact: declare it for owner review rather than keeping it silently.
