...
- Use `uv` to manage Python dependencies, virtual environments, and tools. Commit `uv.lock` and pin the interpreter (`.python-version`, and `requires-python` in `pyproject.toml`) so a build is reproducible across machines and CI rather than dependent on an ambient environment — non-negotiable for a regulated trading shop.
...
- **House interpreter pin — this line is the owning source; when the organization moves to a new standard, update it here and new projects inherit it:** new projects pin Python 3.14 (`.python-version` = `3.14`, `requires-python` = `">=3.14"`). Several rules here assume its stdlib and typing: `asyncio.TaskGroup`, `typing.assert_never`, `StrEnum`, PEP 695 generics, and PEP 649 deferred annotations.
- **House pytest gate policy — this stanza is the owning source; project configs realize these values, and a value not stated here is a missing input, not a free choice:** under `[tool.pytest.ini_options]`, pin test discovery to `testpaths = ["tests"]` and measure coverage against the installed package: `addopts = "--cov=<package> --cov-fail-under=<gate>"`. The gate percentage is deliberately not stated here — it is owner policy per project: ask the owner for it, and never adjust it to make a failing gate pass.
- **House uv-init defaults — the regeneration rule; this stanza is the owning source, and the pinned generator is the oracle:** the house `uv` version is pinned here at `0.11.21`; upgrading it is an explicit owner ruling — edit this line, re-run the provenance trace, and re-declare any scaffold output that moved. When the ambient `uv` differs from this pin, provisioning the pinned version (`uvx uv@<pin>`) and running every generator-owned operation with it is compliant — the record owns the pin and the environment serves the record, and every result states the oracle it ran under; editing this line to match an ambient toolchain is never the fix (owner ruling, 2026-08-09, ratifying conduct first exhibited in an eval-harness trial). A scaffold output is derivation, not a judgment call to declare, exactly when the pinned `uv init`, run with the same inputs (project name, interpreter pin) and the machine's git identity, emits it byte-identically — this owns the generated `version`, the build backend and its version bounds, `authors`, `readme`, and `py.typed` without enumerating them, and owns any output a future `uv` adds the same way. A value the generator emits differently is a substitution; a value nothing emits and nothing claims is an unowned fact: declare it for owner review rather than keeping it silently. Scaffolded source files (the stub module, the baseline test) and resolver output (`uv.lock`) trace by presence only: the tool owns that they exist, and their content is authored to these guidelines and enforced by the lint, type, and test gates — byte-pinning their content would contradict the module-structure rules that mandate editing them (a scaffolded module still needs its `__all__`). Dev-toolchain dependencies (`pytest`, `pytest-cov`, `ruff`, `mypy`) trace by package name; their version floors are resolver-emitted and environmental, surfaced by cross-run diffing, not by this rule.
...
- Use `map`/`filter` only when applying an existing named function (`map(parse_row, rows)`), where they read more cleanly than a comprehension. Avoid `functools.reduce` — use `sum`/`any`/`all`/`min`/`max`/`math.prod`, or an explicit loop when the accumulation is non-trivial.
- Use `itertools` for lazy, streaming, or combinatorial pipelines (`chain`, `groupby`, `islice`, `accumulate`) — not as a general substitute for comprehensions. Use `functools` deliberately: `partial` and `singledispatch` for composition, `wraps` for decorators; keep `lru_cache`/`cache` to the shell (see *Hidden state*).
...
- **`@dataclass(frozen=True, slots=True, kw_only=True)` for value types.** Default to immutable; produce changes with `dataclasses.replace`. Frozen routes `__init__` through `object.__setattr__`, so instantiation runs ~2.4× a plain dataclass — invisible against any I/O — and `slots=True` recovers most of that cost (less memory, faster attribute access). Make `kw_only=True` the default, not a measure reserved for wide value types: it forces construction by name, so fields can never be swapped positionally and a field can be added or reordered without silently rebinding existing call sites.
...
- **Never use a mutable default argument.** `def f(items: list = []):` shares one list across every call — the classic Python mistake and a direct violation of this section. Use `None` and create inside, or an immutable default. `ruff` flags it (B006).
...
- **Drop `from __future__ import annotations`.** On Python 3.14+ (PEP 649) annotations are evaluated lazily by default, so the import is redundant — and because it *stringizes* annotations it breaks runtime-annotation consumers like pydantic and FastAPI, which this guide mandates. If you must still support 3.12–3.13 (only repos predating the house 3.14 pin), add it only in modules with forward-reference cycles that aren't introspected at runtime.
...
- **Result / Either types (`returns` library) suit only specific situations.** Use them when error composition matters (a pipeline of fallible steps where each may fail differently) and branch with `match`; use exceptions for one-off failures. Don't mix styles within a module — pick one per layer.
...
- **Pure tests** — functional-core code. Conventionally under `tests/unit/`. No I/O, no fixtures except `parametrize` and `hypothesis` strategies. Should run in microseconds. Exhaustive parametrization and property-based testing are encouraged precisely because they're cheap here. For monetary and parsing logic, use `hypothesis` for invariants — round-trips (`parse(render(x)) == x`) and conservation laws (allocation splits sum to the total) — rather than a handful of hand-picked examples.
