...
- Use `pnpm` to manage dependencies and workspaces. It is strict by default — no phantom dependencies, so a package you didn't declare can't accidentally resolve — and its content-addressed store keeps installs fast and reproducible against a committed `pnpm-lock.yaml`.
...
- **A helper should run with no build step.** On Node, run a standalone `.ts` file with `tsx script.ts` (or `node --import tsx script.ts`); Deno and Bun execute TypeScript directly (`deno run script.ts`, `bun script.ts`). There is no `tsc` artifact to forget about and no stale `dist/` to debug against.
...
- **Imports go inward, never reverse.** Pure modules MUST NOT import from modules that do I/O, hold state, or call runtime/framework APIs. If you need to, the boundary is in the wrong place — push the I/O out, not the import in.
...
- **`strict: true`, and then more.** Also enable `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`, `noImplicitOverride`, `noFallthroughCasesInSwitch`, `noImplicitReturns`, `isolatedModules`, and `verbatimModuleSyntax`. Treat new type errors as build breaks, not warnings.
- **Lint enforces the architecture.** Use `typescript-eslint` with a type-checked config (or Biome where its rule set suffices) and turn on the rules that encode this document: `no-floating-promises`, `no-explicit-any`, `no-non-null-assertion`, `switch-exhaustiveness-check`, `consistent-type-imports`, `no-unnecessary-condition`, `no-unsafe-*`. A rule that fires is the guideline talking.
...
- **Parse at the edge, compute in the core.** Validate external data — from `fetch`, a file, `JSON.parse` — into a known typed shape once at the boundary with a schema (zod or valibot), then let the core trust the type. "Parse, don't validate."
- **No floating promises.** Every promise is `await`ed or its rejection explicitly handled; `no-floating-promises` enforces it. Thread an injected `AbortSignal` for cancellation rather than firing and forgetting. Concurrency that does I/O (`Promise.all` over `fetch` calls) is a shell concern — quarantine it there; pure transformations stay sync and trivially testable.
...
export type ParseError =
  | { readonly kind: "missing-qty" }
  | { readonly kind: "bad-number"; readonly field: "qty" | "px" };

// Failures are values, not exceptions, in the core.
export function parseFill(raw: Record<string, unknown>): Result<Fill, ParseError> {
  if (raw.qty == null) return { ok: false, error: { kind: "missing-qty" } };
  const qty = Number(raw.qty);
  if (!Number.isInteger(qty)) return { ok: false, error: { kind: "bad-number", field: "qty" } };
  const px = Number(raw.px);
  if (Number.isNaN(px)) return { ok: false, error: { kind: "bad-number", field: "px" } };
  return { ok: true, value: { sym: String(raw.sym), qty, px } };
}
```
...
import { classifySessions, parseFill, type Session, type SessionId } from "../src/trades.js";

const sid = (s: string): SessionId => s as SessionId; // smart-constructor stand-in for the test
...
describe("parseFill", () => {
  ...
  // Error path: pin the specific failure value, not just "it threw".
  test("missing qty", () => {
    expect(parseFill({ sym: "AAPL", px: "190.5" })).toEqual({ ok: false, error: { kind: "missing-qty" } });
  });
});
```
